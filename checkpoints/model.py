...
class BandPassFilter(nn.Module):
    ...
    def __init__(self, kernel_size, low_hz, high_hz, sample_rate=16000,
        ...
        self.n_ = 2 * math.pi * torch.arange(-n, 0).view(1,
                                                         -1) / self.sample_rate  # Due to symmetry, I only need half of the time axes
        self.filters = self.get_band_pass_filter(low_hz, high_hz)
    ...
    def get_band_pass_filter(self, low, high):
        low = torch.tensor(low).unsqueeze(0).unsqueeze(0)
        high = torch.tensor(high).unsqueeze(0).unsqueeze(0)
        band = (high - low)[:, 0]
